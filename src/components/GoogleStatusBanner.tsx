...
export function GoogleStatusBanner() {
  ...
  const runTestWrite = useCallback(async () => {
    ...
    try {
      ...
      const data = (await res.json()) as { ok: boolean; error?: string; cell?: string; value?: string }
      if (data.ok) {
      ...
      setTest({ kind: 'error', message: err instanceof Error ? err.message : 'Network error' })
    }
  }, [])
  ...
  return (
    <div className={`mb-4 rounded-md border ${tone.border} ${tone.bg} ${tone.text} px-3 py-1.5 text-xs`}>
      <div className="flex flex-wrap items-center gap-x-4 gap-y-1">
        <Pill label="Drive" status={drive} detail={drive.ok ? drive.folderName : drive.error} />
        <Pill label="Sheets" status={sheets} detail={sheets.ok ? sheets.title : sheets.error} />

        <div className="ml-auto flex items-center gap-3">
          {test.kind === 'ok' && (
            <span className="text-emerald-700" title={`Wrote "${test.value}" to ${test.cell}`}>
              ✓ wrote to {test.cell}
            </span>
          )}
          {test.kind === 'error' && (
            <span className="text-red-700 truncate max-w-[20rem]" title={test.message}>
              ✗ {test.message}
            </span>
          )}
...
function Pill({ label, status, detail }: { label: string; status: ServiceStatus; detail?: string }) {
  return (
