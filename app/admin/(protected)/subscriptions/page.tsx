...
import { CopyIcon, PencilIcon, PlusIcon, Trash, Loader } from "lucide-react";
...
export default function AdminSubscriptionsPage() {
  ...
  useEffect(() => {
    fetch("/api/admin/subscriptions")
      .then((r) => r.json())
      .then(setPlans);
    fetch("/api/admin/subscriptions/enrollments")
      .then((r) => r.json())
      .then(setEnrollments);
  }, []);
  ...
  async function handleSavePlan(e: React.FormEvent) {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setNotice(
          editingPlan
            ? `"${plan.name}" was updated.`
            : `"${plan.name}" was created. Copy its subscription link from the plan row.`,
        );
      } else {
        ...
        setFormError(
          result?.error ??
            `Could not ${editingPlan ? "update" : "create"} plan. Please check the fields and try again.`,
        );
      }
    } finally {
      setSaving(false);
  ...
  async function handleDeletePlan(id: string, name: string) {
    if (
      !confirm(
        `Delete "${name}"? If it has subscription history, it will be deactivated instead to preserve records.`,
      )
    )
      return;
    setDeleting(id);
    ...
    try {
      ...
      setNotice(
        `"${name}" has subscription history, so it was deactivated instead of deleted.`,
      );
    } finally {
      setDeleting(null);
    }
    ...
    <div>
          ...
          >
            ...
            <DialogContent>
              ...
              <form onSubmit={handleSavePlan} className="space-y-4">
                ...
                <div>
                  <Label htmlFor="name" className="admin-field text-sm">
                    Plan Name
                  </Label>
                  <Input
                    id="name"
                ...
                <div>
                  <Label htmlFor="desc" className="admin-field text-sm">
                    Description
                  </Label>
                  <Input
                    id="desc"
                ...
                <div className="grid gap-3 sm:grid-cols-2">
                  <div>
                    <Label htmlFor="amount" className="admin-field text-sm">
                      Monthly Amount (PHP)
                    </Label>
                    <Input
                      id="amount"
                    ...
                    <p className="admin-muted mt-1 text-xs">
                      This is charged every month.
                    </p>
                  </div>
                  <div>
                    <Label htmlFor="duration" className="admin-field text-sm">
                      Auto-Debit For How Many Months?
                    </Label>
                    <Input
                      id="duration"
                      type="number"
                    ...
                    />
                    <p className="admin-muted mt-1 text-xs">
                      Example: 12 means monthly charges for 12 months.
                    </p>
                  </div>
                </div>
                <div className="admin-alert-warning rounded-lg px-4 py-3 text-xs">
                  Subscriber will pay ₱
                  {Number(form.amount || 0).toLocaleString("en-PH", {
                    minimumFractionDigits: 2,
                  })}{" "}
                  every month for {Number(form.durationMonths || 0)} month
                  {Number(form.durationMonths) === 1 ? "" : "s"}.
                </div>
                <DialogFooter>
                  <Button
                    type="button"
      ...
      {tab === "plans" && (
        <div className="admin-panel overflow-hidden rounded-2xl">
          {plans.length === 0 ? (
            <div className="admin-empty p-12 text-center">
                ...
                <tbody className="admin-table-body divide-y">
                  {enrollments.map((sub) => (
                    ...
                    >
                      ...
                      <td className="admin-text px-6 py-4 text-right font-medium">
                        ₱
                        {sub.plan.amount.toLocaleString("en-PH", {
                          minimumFractionDigits: 2,
                        })}
                      </td>
                      <td className="px-6 py-4 text-center">
